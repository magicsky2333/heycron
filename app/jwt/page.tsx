  ...
  return (
    <main className="min-h-screen bg-[#080812] text-white antialiased">
      ...
      <section className="max-w-3xl mx-auto px-6 pt-12 pb-6 text-center">
        ...
        <p className="text-gray-400">粘贴 JWT Token，查看 Header、Payload 内容和过期时间</p>
      </section>
      ...
      <section className="max-w-3xl mx-auto px-6 pb-16 space-y-4">
        ...
        <div>
          <textarea
            ...
            className="w-full h-32 bg-gray-900 border border-white/10 rounded-xl p-4 text-sm font-mono text-white placeholder-gray-600 focus:outline-none focus:border-indigo-500 transition-colors resize-none"
            spellCheck={false}
          />
        </div>
        ...
        <button
          onClick={decode}
          disabled={!input.trim()}
          className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 py-3 rounded-lg font-semibold transition-colors cursor-pointer"
        >
          解码 →
        </button>
        ...
        {result && (
          <div className="space-y-4">
            ...
            {/* Header */}
            <div className="bg-gray-900 border border-white/10 rounded-xl p-5 group">
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs text-gray-500 uppercase tracking-wider">Header</span>
                <button
                  onClick={() => handleCopy(JSON.stringify(result.header, null, 2), 'header')}
                  className="text-xs text-gray-500 hover:text-white opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                >
            ...
            {/* Signature note */}
            <div className="bg-gray-900/50 border border-white/5 rounded-xl px-5 py-4">
              <span className="text-xs text-gray-500 uppercase tracking-wider block mb-1">Signature</span>
              <p className="text-gray-600 text-sm">签名部分仅用于服务端验证，客户端无法验证。此工具只解码，不验证签名有效性。</p>
            </div>
          </div>
        )}
      </section>
